...
async def nagato_undo_standalone(
    ...
    # Ensure ctx has required attributes - auto-detect or create fallback
    if ctx is None:
        try:
            from nagato_tools.ctx_mock import MockFSMContext
            import os
            workspace_root = os.path.abspath('.')
            ctx = MockFSMContext(workspace_root=workspace_root)
        except Exception:
            from nagato_tools.ctx_mock import MockFSMContext
            ctx = MockFSMContext()
    ...
    ctx.redo_dir.mkdir(parents=True, exist_ok=True)
